			...
			this._send<ResolveSymbolResponse | UdfErrorResponse>('symbols', params)
				.then((response: ResolveSymbolResponse | UdfErrorResponse) => {
					if ('s' in response) {
						...
						const resolvedResponse = response as ResolveSymbolResponse;
						const symbol = resolvedResponse.name;
						const listedExchange = resolvedResponse.listed_exchange ?? resolvedResponse['exchange-listed'];
						const tradedExchange = resolvedResponse.exchange ?? resolvedResponse['exchange-traded'];
						const fullName = resolvedResponse.full_name ?? `${tradedExchange}:${symbol}`;
						...
						const result: LibrarySymbolInfo = {
							...
							currency_code: resolvedResponse.currency_code ?? resolvedResponse['currency-code'],
							original_currency_code: resolvedResponse.original_currency_code ?? resolvedResponse['original-currency-code'],
							unit_id: resolvedResponse.unit_id ?? resolvedResponse['unit-id'],
							original_unit_id: resolvedResponse.original_unit_id ?? resolvedResponse['original-unit-id'],
							unit_conversion_types: resolvedResponse.unit_conversion_types ?? resolvedResponse['unit-conversion-types'],
							has_intraday: resolvedResponse.has_intraday ?? resolvedResponse['has-intraday'] ?? false,
							visible_plots_set: resolvedResponse.visible_plots_set ?? resolvedResponse['visible-plots-set'],
							minmov: resolvedResponse.minmovement ?? resolvedResponse.minmov ?? 0,
							minmove2: resolvedResponse.minmovement2 ?? resolvedResponse.minmove2,
							session: resolvedResponse.session ?? resolvedResponse['session-regular'],
							session_holidays: resolvedResponse.session_holidays ?? resolvedResponse['session-holidays'],
							supported_resolutions: resolvedResponse.supported_resolutions ?? resolvedResponse['supported-resolutions'] ?? this._configuration.supported_resolutions ?? [],
							has_daily: resolvedResponse.has_daily ?? resolvedResponse['has-daily'] ?? true,
							intraday_multipliers: resolvedResponse.intraday_multipliers ?? resolvedResponse['intraday-multipliers'] ?? ['1', '5', '15', '30', '60'],
							has_weekly_and_monthly: resolvedResponse.has_weekly_and_monthly ?? resolvedResponse['has-weekly-and-monthly'],
							has_empty_bars: resolvedResponse.has_empty_bars ?? resolvedResponse['has-empty-bars'],
							volume_precision: resolvedResponse.volume_precision ?? resolvedResponse['volume-precision'],
							format: resolvedResponse.format ?? 'price',
						};
